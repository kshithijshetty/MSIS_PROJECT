...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_KEY = 'AIzaSyCkRoHm2I1eZRKfwRONZz6R9Iw3FWfIGC0';  // Replace with your actual YouTube Data API key
  // const API_URL = 'http://my-elb-2062136355.us-east-1.elb.amazonaws.com:80';   
  const API_URL = 'http://localhost:5000/';
  
  console.log("Extension initialized. Using API_URL:", API_URL);
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      if (predictions) {
        ...
        // Add the Language Detection section
        outputDiv.innerHTML += `
          <div class="section">
        ...
        // Fetch and display language detection
        await fetchAndDisplayLanguageDetection(comments.map(comment => comment.text));
  ...
  async function getSentimentPredictions(comments) {
    try {
      console.log(`Sending ${comments.length} comments to ${API_URL}/predict_with_timestamps`);
      ...
      if (response.ok) {
        console.log("Predictions received successfully:", result.length);
      ...
      outputDiv.innerHTML += `<p>Error fetching sentiment predictions: ${error.message}</p>`;
      return null;
    }
  }
  ...
  async function fetchAndDisplayLanguageDetection(comments) {
    try {
      const response = await fetch(`${API_URL}/detect_languages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ comments })
      });
      if (!response.ok) {
        throw new Error('Failed to fetch language detection chart');
      }
      const blob = await response.blob();
      const imgURL = URL.createObjectURL(blob);
      const img = document.createElement('img');
      ...
      img.style.width = '100%';
      img.style.marginTop = '20px';
      // Append the image to the language-chart-container div
      const languageChartContainer = document.getElementById('language-chart-container');
      languageChartContainer.appendChild(img);
    } catch (error) {
      console.error("Error fetching language detection chart:", error);
      outputDiv.innerHTML += "<p>Error fetching language detection.</p>";
    }
  }
});
